...
echo "CUSTOMER SERVICE: ENVIRONMENT IS $NODE_ENV $DEVELOPMENT_MODE";

if [ $DEVELOPMENT_MODE != "local" ]; then
  echo "CUSTOMER SERVICE: DEPLOYING PRISMA!";
  npm run --prefix /app prisma:generate;

  echo "CUSTOMER SERVICE: STARTING PRODUCTION SERVER!";
  npm run --prefix /app build;
  npm run --prefix /app start:prod;
else
  if [ ! -d /var/app/node_modules ]; then
    echo "CUSTOMER SERVICE: MOVING NODE MODULES!";
  ...
  echo "CUSTOMER SERVICE: STARTING DEVELOPMENT SERVER!";
